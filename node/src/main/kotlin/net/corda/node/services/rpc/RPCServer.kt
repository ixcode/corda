 ...
 *
 * The way this is done is similar to that in [RPCClient], we use AMQP and add a context to stores the subscription map.
 *
...
class RPCServer(
    ...
    init {
        ...
        opsList.forEach { ops ->
            val groupedMethods = with(ops.javaClass) {
                declaredMethods.groupBy { name + CLASS_METHOD_DIVIDER + it.name }
            }
    ...
    private fun deriveFqMethodName(inMethodName: String) = if (inMethodName.contains(CLASS_METHOD_DIVIDER)) {
        ...
        // Legacy client
        opsList.first().javaClass.name + CLASS_METHOD_DIVIDER + inMethodName
    }
